export default function StatsBar() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 mb-20 sm:mb-32">
      <div className="glass-panel rounded-2xl p-6 md:p-8 flex flex-wrap justify-center md:justify-between gap-8 text-center animated-border-bottom">
        {stats.map((stat, i) => (
          ...
          >
            <p className="text-white font-semibold text-xl tracking-tight group-hover:text-amber-200 transition-colors duration-300">
              {stat.value}
            </p>
            <p className="text-xs text-zinc-500 uppercase tracking-widest mt-1 group-hover:text-zinc-400 transition-colors duration-300">
              {stat.label}
            </p>
          </div>
        ))}
