    ...
    <section className="max-w-5xl mx-auto px-6 mb-32">
      ...
      <div className="grid md:grid-cols-2 gap-x-12 gap-y-6">
        {benefits.map((b, i) => (
          <div
            key={b.title}
            className="flex items-start gap-4 p-6 rounded-xl hover:bg-white/[0.02] transition-all duration-300 group"
            style={{ animationDelay: `${i * 80}ms` }}
          >
            <div className="w-10 h-10 rounded-xl bg-amber-400/[0.08] flex items-center justify-center text-amber-400 shrink-0">
              <b.icon className="w-5 h-5" />
            </div>
            <div>
              <h4 className="text-white font-medium mb-1 group-hover:text-amber-100 transition-colors duration-300">
                {b.title}
              </h4>
